...
def test_download_plugin_member_forbidden_friendly_error():
    """会员插件服务端返回 401/403 时，应提示登录/会员，而不是裸 HTTP 错误。"""
    ...
    try:
        ...
        assert "会员" in str(e) or "登录" in str(e)
...
def test_plugin_access_tiers():
    """商店权益判定：免费 / 永久档解锁全部 / 其余买断 + 已购插件免费版可用。"""
    free = _entry({"tier": "free"})
    member = _entry({"id": "member-pack", "tier": "member"})
    buy = _entry({"id": "pro-pack", "tier": "buy"})

    # 免费插件：未登录也可装
    assert plugin_market.plugin_access(free, {})["allowed"] is True
    assert plugin_market.plugin_access(free, {"full": False})["label"] == "免费"

    # 付费插件：只有「解锁全部付费包」（full=all_plugins）或单独买断才可装
    assert plugin_market.plugin_access(member, {"full": False})["allowed"] is False
    assert "买断" in plugin_market.plugin_access(member, {"full": False})["reason"]
    assert plugin_market.plugin_access(member, {"full": True})["allowed"] is True
    # 月费会员（member=True）也不解锁能力包：必须单独买断
    assert plugin_market.plugin_access(
        member, {"full": False, "member": True})["allowed"] is False

    # 单买插件：单独购买后免费版也可装
    assert plugin_market.plugin_access(buy, {"full": True})["allowed"] is True
    assert plugin_market.plugin_access(buy, {"full": False})["allowed"] is False
    assert plugin_market.plugin_access(
        buy, {"full": False, "owned_plugins": ["pro-pack"]})["allowed"] is True
    assert plugin_market.plugin_access(
        buy, {"full": False, "owned_plugins": ["pro-pack"]})["label"] == "已购"

    # 未知档位按免费处理（兼容旧清单）
    assert plugin_market.plugin_access(_entry({"tier": "unknown"}), {})["allowed"] is True
...
if __name__ == "__main__":
    ...
    test_download_plugin_member_forbidden_friendly_error()
    print("OK download_plugin_member_forbidden_friendly_error")
