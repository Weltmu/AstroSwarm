...
def test_plugins_page_entitlement_gate():
    """插件商店按权益显示标签：免费可装，会员插件锁定并亮「开通会员」。"""
    from PySide6.QtWidgets import QApplication
    ...
    assert any("[免费]" in t for t in texts), texts
    assert any("[买断]" in t for t in texts), texts

    idx_member = next(i for i, t in enumerate(texts) if "[买断]" in t)
    pp.market_list.setCurrentRow(idx_member)
    app.processEvents()
    assert pp.btn_market_buy.isEnabled(), "选中付费能力包应亮起「购买能力包」"

    idx_free = next(i for i, t in enumerate(texts) if "[免费]" in t)
    pp.market_list.setCurrentRow(idx_free)
    app.processEvents()
    assert not pp.btn_market_buy.isEnabled(), "选中免费插件不应显示开通入口"
    pp.deleteLater()
    print("OK ui plugins_page_entitlement_gate")
...
if __name__ == "__main__":
    ...
    test_plugins_page_entitlement_gate()
