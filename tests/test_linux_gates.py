...
def test_allowed_packs_monthly_only_owned(cfg_home):
    """月费档：只有单独买断的包会加载，其余付费包保持冻结。"""
    _install_packs(cfg_home / "data", "eva", "proactive", "timer", "memory")
    _set_plan("monthly", FUTURE, owned=["timer"])
    allowed = deploy._allowed_packs()
    assert "eva" in allowed            # 免费引流包始终放行
    assert "timer" in allowed          # 明确买过
    assert "proactive" not in allowed, allowed
    assert "memory" not in allowed, allowed


def test_allowed_packs_trial_only_free(cfg_home):
    """试用期不是会员：磁盘上已装的付费包一律不加载。"""
    _install_packs(cfg_home / "data", "eva", "proactive", "group-manager")
    _set_plan("trial", FUTURE)
    allowed = deploy._allowed_packs()
    assert allowed == ["eva"], allowed


def test_allowed_packs_permanent_loads_all(cfg_home):
    _install_packs(cfg_home / "data", "eva", "proactive", "memory")
    _set_plan("permanent")
    allowed = deploy._allowed_packs()
    assert set(allowed) == {"eva", "proactive", "memory"}, allowed


def test_allowed_packs_unsigned_blocks_paid(cfg_home, monkeypatch):
    """没有签名时，即使 config.json 写着 permanent 也只放免费包。"""
    monkeypatch.setattr(auth, "verify_entitlement", lambda cfg=None: False)
...
# ------------------------------------------------------------------ _entitled
def test_entitled_monthly_only_owned(cfg_home):
    _set_plan("monthly", FUTURE, owned=["timer"])
    assert tools._entitled("timer") is True
    assert tools._entitled("proactive") is False


def test_entitled_trial_denies_paid(cfg_home):
    _set_plan("trial", FUTURE)
    assert tools._entitled("proactive") is False


def test_entitled_permanent_allows_all(cfg_home):
    _set_plan("permanent")
    assert tools._entitled("proactive") is True


def test_entitled_unsigned_denies(cfg_home, monkeypatch):
    monkeypatch.setattr(auth, "verify_entitlement", lambda cfg=None: False)
    _set_plan("permanent", 0, owned=["timer"])
    assert tools._entitled("timer") is False
