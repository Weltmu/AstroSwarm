...
导出规则（只取被 git 跟踪的文件）：
    * 付费能力包源码：src/qbotmanager/assets/tool_packs/<付费包 id>/
...
# 付费能力包源码：可能在顶层，也可能嵌在免费包里（如 tool_packs/eva/bundled/reply-rhythm），
# 所以先吃掉中间的任意目录段，再要求出现付费包 id
PAID_PACK_RE = re.compile(
    r"/tool_packs/(?:[^/]+/)*(?:proactive|memory|knowledge|web-search|timer|group-manager|reply-rhythm)/"
)
