...
def _entitled(plugin_id: str) -> bool:
    ...
    权益必须带账号服务签名；没有签名（或签名不对）一律按免费版。
    全解锁只认 plans.json 里 all_plugins=true（目前只有 permanent）且未过期；
    其余档位（含 monthly/quarterly/yearly）和 trial 只认 owned_plugins 里明确买过的包。
    """
    cfg = headless_config.load()
    if not auth.verify_entitlement(cfg):
        return False
    ...
    if pid in owned:
        return True
    return bool(auth.feature_gate(cfg).get("full"))
...
def install(entry: dict) -> dict:
    ...
    if not _entitled(pid):
        raise PermissionError("当前为免费版或未购买该插件")
