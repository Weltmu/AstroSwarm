...
def _allowed_packs() -> list:
    """已购或档位全包含的已装工具包（会员到期=冻结，不加载但保留文件）。

    安全（务必保持）：这个列表决定**付费能力包会不会被加载**，所以
    plan / owned_plugins 都可能是用户改过的 config.json 里的值，必须先用
    账号服务的 Ed25519 签名验一遍（auth.verify_entitlement），验不过一律只放免费包。

    「全解锁」口径必须读 plans.json：只有该档 all_plugins=true（目前只有 permanent）
    才算全解锁；其余档位（含 monthly/quarterly/yearly）和 trial 只放行 owned_plugins
    里明确买过的包 —— 与服务端下载口的口径保持一致。
    """
    cfg = headless_config.load()
    owned = {str(x).strip() for x in (cfg.get("owned_plugins") or []) if str(x).strip()}
    ...
    try:
        ...
        verified = bool(auth_mod.verify_entitlement(cfg))
        unlocked_all = bool(auth_mod.feature_gate(cfg).get("full"))
    except Exception:  # noqa: BLE001
        verified = False
        unlocked_all = False
    ...
    if root.exists():
        for pack in root.iterdir():
            ...
            if manifest.exists():
                ...
                if not mid:
                    continue
                if mid in tool_packs_mod.FREE_PACK_IDS:
                    allowed.append(mid)          # 免费引流包：任何授权状态都放行
                elif verified and (mid in owned or unlocked_all):
                    allowed.append(mid)          # 付费包：验签通过且「已购」或「档位全包含」
    return allowed
