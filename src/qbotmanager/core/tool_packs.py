# -*- coding: utf-8 -*-
"""桌面端能力包管理：下载安装/卸载工具包、人设包、行为包（含会员/单买权益校验）。
...
AgentRuntime 按权益加载（会员到期 = 冻结，文件保留但不加载）。
"""
import json
import shutil
import zipfile
...
# 免费引流包：任何授权状态下都允许加载（与市场清单 tier=free 保持一致）。
# 这几个包不参与付费门，否则免费版用户装完人格包也用不了。
FREE_PACK_IDS = {"eva", "liqinghan", "qweather"}


def _entitled(settings, plugin_id: str) -> bool:
    if plugin_id in FREE_PACK_IDS:
        return True
    ent = lic_mod.entitlements()
    owned = {str(x).strip() for x in (ent.get("owned_plugins") or [])}
    return bool(ent.get("full")) or (plugin_id in owned)
...
def install_pack(settings, entry: dict, zip_path=None, log=None) -> dict:
    """下载并安装能力包；权益不足抛 PermissionError。"""
