...
# 商店权益档位：free = 免费安装；member = 会员期间可用；buy = 需单独购买（会员期间同样可用）
TIER_FREE = "free"
TIER_MEMBER = "member"
TIER_BUY = "buy"
...
def entry_tier(entry: dict) -> str:
    """返回市场条目的权益档位（缺失/未知一律按免费处理）。"""
    tier = str((entry or {}).get("tier") or "").strip().lower()
    return tier if tier in (TIER_FREE, TIER_MEMBER, TIER_BUY) else TIER_FREE


def plugin_access(entry: dict, entitlements: dict | None = None) -> dict:
    """判断当前账号能否安装该插件。

    entitlements 来自 license.entitlements()（plan / full(=all_plugins) / owned_plugins）。
    规则（与无头端 `deploy._allowed_packs` 同一口径）：
    - 已单独购买（owned_plugins 含该 id）→ 免费版也可装，永久有效；
    - tier=free → 直接可装；
    - `full`（plans.json 里 all_plugins=true 的档位，目前只有永久档）→ 全部付费包可装；
    - 其余（含月费会员）→ 需要单独买断该插件。
    """
    entitlements = entitlements or {}
    plugin_id = str((entry or {}).get("id") or "").strip()
    owned = {
        str(x).strip() for x in (entitlements.get("owned_plugins") or []) if str(x).strip()
    }
    tier = entry_tier(entry)
    if plugin_id and plugin_id in owned:
        return {"allowed": True, "reason": "已单独购买", "label": "已购"}
    if tier == TIER_FREE:
        return {"allowed": True, "reason": "免费", "label": "免费"}
    if bool(entitlements.get("full")):
        return {"allowed": True, "reason": "永久档已包含全部付费包", "label": "已解锁"}
    return {
        "allowed": False,
        "reason": "能力包是买断制：单独购买后永久可用（月费档不含能力包）",
        "label": "买断",
    }
...
def download_plugin(url: str, dest, sha256: str | None = None,
    ...
    try:
        ...
        if e.code in (401, 403):
            raise RuntimeError("该插件需要登录星群账号（会员或单独购买）后才能下载") from e
