...
async def send_forward_message(user_id: int = None, group_id: int = None, messages: list = None):
    ...
    try:
        ...
        if not user_id and not group_id:
            logger.error("参数错误: 至少指定 user_id 或 group_id 其中一个")
            return None
...
async def send_platform_text(event, title: str, content: str) -> bool:
    """跨平台长文本发送：QQ 保持合并转发；微信 ClawBot 直接发文本。"""
...
async def send_help_document(title: str, content: str, event: MessageEvent) -> bool:
    """
    尝试以文档（群文件/私聊文件）形式发送指令帮助。
    成功返回 True（不再发送聊天记录版帮助）；失败返回 False，由调用方回退到聊天记录形式。
    """
    bot = get_bot()
    try:
        path = store.get_plugin_data_dir() / "ac_help.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(f"# {title}\n\n{content}\n", encoding="utf-8")
        file_name = "AC_Chat指令帮助.md"
        ...
        logger.info("指令帮助文档发送成功")
        return True
    except Exception as e:
        logger.warning(f"指令帮助文档发送失败，回退到聊天记录形式: {e}")
        return False
