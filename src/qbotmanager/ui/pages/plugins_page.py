...
class PluginsPage(QWidget):
    ...
    def _build_ui(self):
        ...
        sv.addWidget(self._note(
            ...
            "免费插件人人可装；付费能力包是买断制：单独购买后永久可用（永久档已包含全部）。"))
        ...
        self.market_list.currentItemChanged.connect(
            lambda *_: self._update_market_buttons())
        ...
        self.btn_market_buy = QPushButton("购买能力包")
        self.btn_market_buy.setObjectName("ghost")
        self.btn_market_buy.setEnabled(False)
        self.btn_market_buy.clicked.connect(self._open_buy)
        ...
        m_row.addWidget(self.btn_market_buy)
        ...
        # 以前桌面端只有「购买能力包」一个入口，兑换码（活动/人工发放）根本没法用，
        # 用户只能在 Linux 端或网页控制台兑换。
        r_row = QHBoxLayout()
        r_row.setSpacing(8)
        self.redeem_edit = QLineEdit()
        self.redeem_edit.setPlaceholderText("兑换码（官网购买 / 活动发放的码）")
        ...
        self.btn_ent_refresh.setToolTip(
            "刚买完插件点这里：去星群账号重新拉一次权益。\n"
            "本机没登录账号时先点「购买能力包」旁边的账号入口登录。")
        ...
        pv.addWidget(self._note(
            "买断制：单独购买后永久可用，不按月扣费。选中后可看详情、改参数；"
            "卸载只影响本机，重启机器人后不再加载。"))
    ...
    def _redeem_code(self):
        ...
        兑换成功后必须再拉一次 /me：兑换接口不回签名，而「有没有签名」决定付费能力
        能不能用，只落 plan 不落签名等于白兑。
        """
    ...
    def _on_market_loaded(self, res):
        ...
        if acc:
            plan_txt = "永久档" if ent.get("full") else "免费版"
            if ent.get("owned_plugins"):
                plan_txt += f" · 已购 {len(ent['owned_plugins'])} 个插件"
            plan_label = "当前账号：" + plan_txt
        else:
            plan_label = "未登录星群账号（仅能安装免费插件）"
        self.ent_status.setText(plan_label)
        locked = sum(
            1 for e in entries.values()
            if not plugin_market.plugin_access(e, ent).get("allowed"))
        self.market_status.setText(
            f"插件商店已加载：{len(entries)} 个 · {plan_label}"
            + (f" · {locked} 个需买断" if locked else "")
    ...
    def _refresh_market_list(self):
        ...
        for eid, e in self._market_entries.items():
            ...
            tip += "\n\n" + (
                "可安装：" + access["reason"]
                if access["allowed"] else "不可安装：" + access["reason"]
            )
            item.setToolTip(tip)
            if not access["allowed"]:
                item.setForeground(QColor("#94A3B8"))
            elif access["label"] == "免费":
                item.setForeground(QColor("#34D399"))
            elif access["label"] == "已购":
    ...
    def _update_market_buttons(self):
        """选中需买断的插件时亮起「购买能力包」（买断后永久可用）。"""
        item = self.market_list.currentItem()
        locked = False
        if item is not None:
            e = self._market_entries.get(str(item.data(Qt.UserRole) or ""))
            if e:
                locked = not plugin_market.plugin_access(
                    e, self._entitlements).get("allowed")
        self.btn_market_buy.setEnabled(locked)

    def _open_buy(self):
        QDesktopServices.openUrl(QUrl(lic_mod.PAY_URL))
    ...
    def _install_market_plugin(self):
        ...
        if not access.get("allowed"):
            ret = QMessageBox.question(
                self, "需要买断",
                f"「{e.get('name')}」{access.get('reason')}。\n\n"
                "是否去购买？（买断后永久可用）",
